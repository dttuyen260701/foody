...
import com.example.foody.fragment.ReviewsFragment;
...
public class FoodAdapter extends RecyclerView.Adapter<FoodAdapter.FoodViewHolder> {
    ...
    public void onBindViewHolder(@NonNull FoodViewHolder holder, int position) {
        ...
        if(!for_Search && index > -1 || for_Search){
            if(foods_list.get(index).isStatus())
                holder.imgFood_item.setImageBitmap(foods_list.get(index).getImage_Food());
            else
                holder.imgFood_item.setImageResource(R.drawable.sold_out);
            int hinh = (foods_list.get(index).is_Favorite()) ? R.drawable.liked_icon : R.drawable.like_icon;
            holder.txtName_Food_item.setText(foods_list.get(index).getName_Food());
            holder.imgFavorite_Food_item.setImageResource(hinh);
            ...
            holder.txtDescri_Food_item.setText(descrip);

            holder.txtTime_Cook_Food_item.setText(
            ...
            holder.imgFavorite_Food_item.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if(Constant_Values.getIdCus() == -1){
                        Toast.makeText(context.getApplicationContext(), "Vui lòng đăng nhập để thêm vào danh sách thích",
                        ...
                        if(foods_list.get(index).is_Favorite()){
                            //xóa
                            listener.insert_or_del_Fav(fav, false, holder,
                                    R.drawable.like_icon, foods_list, index, false);

                        } else {
                            //thêm
                            listener.insert_or_del_Fav(fav, true, holder,
                                    R.drawable.liked_icon, foods_list, index, true);
                        }
                    }
                }
            });
            ...
            if(bill_details.getCount() < 1){
                holder.btnRedu_FoodRow.setVisibility(View.GONE);
                holder.txt_count_FoodRow.setText("");
            } else {
                holder.btnRedu_FoodRow.setVisibility(View.VISIBLE);
                holder.txt_count_FoodRow.setText("" + bill_details.getCount());
            }

            holder.btnRedu_FoodRow.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    ...
                    if(count == -1){
                        ...
                        holder.txt_count_FoodRow.setText("" + (count));
                    }
                }
            });

            holder.btnInc_FoodRow.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
            ...
            holder.layout_Food_Row.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
    ...
    public class FoodViewHolder extends RecyclerView.ViewHolder{
        ...
        public FoodViewHolder(@NonNull View itemView) {
            ...
            imgFavorite_Food_item = (ImageView) itemView.findViewById(R.id.imgFavorite_Food_item);

            txtName_Food_item = (TextView) itemView.findViewById(R.id.txtName_Food_item);
            txtDescri_Food_item = (TextView) itemView.findViewById(R.id.txtDescri_Food_item);
            txtPrice_Food_item = (TextView) itemView.findViewById(R.id.txtPrice_Food_item);
            txtRate_Food_item = (TextView) itemView.findViewById(R.id.txtRate_Food_item);
            txtTime_Cook_Food_item = (TextView) itemView.findViewById(R.id.txtTime_Cook_Food_item);
